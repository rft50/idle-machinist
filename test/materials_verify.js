"use strict"
{
	...
	function verifyPlus(prop, rule, msg) {
		if (!rule(prop)) {
			tip += "\n" + msg
			valid = false
			allValid = false
	...
	var color = x => (x >= 0 && x <= 255)
	var material = x => (x == "wood" || x == "metal" || x == "crystal")
